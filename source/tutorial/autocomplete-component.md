...
The `handleFilterEntry` action will assign `results` with the list of rentals it returns. The `{{yield}}` helper will then use `results` to fill in the value of our block parameter, `|filteredResults|`. We still need to define how `handleFilterEntry` gets those results though, so let's do that now:
